...
myEigen_QR <- function(A, numIter = 1000){
  ...
  rows <- dim(A)[1]
  columns <- dim(A)[2]
  v <- matrix(rnorm(rows * rows), nrow = rows)
  temp <- v
  ...
  return(list("D" = diag(outputR), "V" = outputQ))
...
#    print(my_coef)
#    newr <- 3
#    x <- matrix(rnorm(newr * newr), nrow = newr)
#    print(myEigen_QR(t(x) %*% x, 1000))
#    print(eigen(t(x) %*% x))
# 
#    # newr <- 3
#    # x <- matrix(rnorm(newr * newr), nrow = newr)
#    # print(myEigen_QRC(t(x) %*% x, 1000))
#    # print(eigen(t(x) %*% x))
